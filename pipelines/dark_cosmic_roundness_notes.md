...
4. Keep the **best** normalized correlation as `gaussian_like_score`, its sigma as
   `gaussian_sigma_pixels`, and fitted FWHM diameter as `gaussian_diameter_pixels`.
5. Reject tiny one-pixel-like events by requiring `gaussian_diameter_pixels >= 3`.

Circularity is still recorded as a diagnostic, but selection/ranking is gaussian-only.
Stats plots include a **red reference marker** for a perfect 9-pixel-diameter PSF-like source.
...
print(roundest["detector", "footprint_id", "gaussian_like_score", "gaussian_sigma_pixels", "gaussian_diameter_pixels", "selected_cutout_path"][:20])
