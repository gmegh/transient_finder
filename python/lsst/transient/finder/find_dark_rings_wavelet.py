...
class FindDarkRingsWaveletTask(pipeBase.PipelineTask):
    ...
    def _gaussian_like_score(self, stamp: np.ndarray) -> tuple[float, float]:
        """Return (best_correlation, best_sigma_pixels) with adaptive sigma scan."""
        if stamp.size == 0:
            return 0.0, np.nan
    ...
    def run(self, inputExp: Exposure, instrument: str, exposure: int, detector: int) -> pipeBase.Struct:
        ...
        for i, fp in enumerate(footprints):
            ...
            self._save_stamp(stamp, all_path, f"fp={i} c={circularity:.3f} g={gaussian_like:.3f} s={gaussian_sigma:.2f}")
        ...
        catalog = Table(rows=rows) if rows else Table(
            ...
            dtype=[int, int, int, int, float, float, float, bool, float, float, float, int, int, int, int],
        )
