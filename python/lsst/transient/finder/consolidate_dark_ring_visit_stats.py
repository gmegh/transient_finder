...
class ConsolidateDarkRingVisitStatsConfig(
    ...
    minCircularity = pexConfig.Field(dtype=float, default=0.6, doc="Minimum circularity for selected candidates.")
...
class ConsolidateDarkRingVisitStatsTask(pipeBase.PipelineTask):
    ...
    def runQuantum(self, butlerQC, inputRefs, outputRefs):  # type: ignore[no-untyped-def]
        ...
        if len(catalog) > 0:
            ...
            sc = ax.scatter(area[finite], circ[finite], c=gaus[finite], s=5, alpha=0.35, cmap="viridis")
            ax.set_xlabel("footprint area [pix]")
            ax.set_ylabel("circularity")
            ax.set_title("Per-visit candidates: area vs circularity")
            fig.colorbar(sc, ax=ax, label="gaussian_like_score")
            fig.tight_layout()
            p_sc = stats_dir / "area_vs_circularity.png"
            fig.savefig(p_sc, dpi=140, bbox_inches="tight")
            plt.close(fig)
            stats_rows.append({"kind": "scatter", "path": str(p_sc), "footprint_id": -1})

            fig, ax = plt.subplots(figsize=(6, 4))
            ax.hist(circ[np.isfinite(circ)], bins=80, alpha=0.85, color="tab:blue")
            ax.set_xlabel("circularity")
            ax.set_ylabel("N footprints")
            ax.set_title("Per-visit circularity histogram")
            fig.tight_layout()
            p_hist = stats_dir / "circularity_hist.png"
            ...
            select = (
                np.isfinite(circ)
                & np.isfinite(gaus)
                & (circ >= self.config.minCircularity)
                & (gaus >= self.config.minGaussianLike)
            )
            selected = catalog[select]
            if len(selected) > 0:
                score = np.asarray(selected["circularity"]) * np.asarray(selected["gaussian_like_score"])
